...
from constants import DEEPSPCAS9_MODEL_DIR

# ---------- External: DeepSpCas9 model definition and scoring function ----------
# --- DeepSpCas9 Score Calculation from DeepPrime ---
class _Deep_SpCas9(object):
    def __init__(self, filter_size, filter_num, node_1=80, node_2=60, l_rate=0.005):
        ...
        def create_new_conv_layer(
            ...
            w = tf.compat.v1.Variable(
                tf.compat.v1.truncated_normal(conv_filt_shape, stddev=0.03),
                name=name + "_W",
            )
            b = tf.compat.v1.Variable(
                tf.compat.v1.truncated_normal([num_filters]), name=name + "_b"
            )
        ...
        L_pool_0 = create_new_conv_layer(
            self.inputs, 4, filter_num[0], [1, filter_size[0]], [1, 2], name="conv1"
        )
        L_pool_1 = create_new_conv_layer(
            self.inputs, 4, filter_num[1], [1, filter_size[1]], [1, 2], name="conv2"
        )
        L_pool_2 = create_new_conv_layer(
            self.inputs, 4, filter_num[2], [1, filter_size[2]], [1, 2], name="conv3"
        )
        ...
        with tf.compat.v1.variable_scope("Fully_Connected_Layer1"):
            ...
            L_flatten = tf.concat(
                [L_flatten_0, L_flatten_1, L_flatten_2], 1, name="concat"
            )
        ...
        self.optimizer = tf.compat.v1.train.AdamOptimizer(l_rate).minimize(
            self.obj_loss
        )


# class end: Deep_xCas9
def _Model_Finaltest(sess, TEST_X, model):
    test_batch = 500
    test_spearman = 0.0
    ...
    for i in range(int(np.ceil(float(TEST_X.shape[0]) / float(test_batch)))):
        Dict = {
            model.inputs: TEST_X[i * test_batch : (i + 1) * test_batch],
            model.is_training: False,
        }
        TEST_Z[i * test_batch : (i + 1) * test_batch] = sess.run(
            [model.outputs], feed_dict=Dict
        )[0]
...
def _calculate_DeepSpCas9_score(list_target30):
    ...
    with tf.compat.v1.Session(config=conf) as sess:
        ...
        model = _Deep_SpCas9(filter_size, filter_num, node_1, node_2, args[2])

        saver = tf.compat.v1.train.Saver()
        saver.restore(sess, DEEPSPCAS9_MODEL_DIR.joinpath(best_model + ".ckpt"))
        list_score = _Model_Finaltest(sess, TEST_X, model)
