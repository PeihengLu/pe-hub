...
from pe_ensemble.evaluation.runner import _evaluation_skip_reason, execute_evaluation
...
def test_execute_evaluation_marks_job_skipped_for_non_standardizable(tmp_path, monkeypatch):
    ...
    with patch("pe_ensemble.evaluation.runner.fetch_model_format_result", return_value=empty_fetch):
...
def test_execute_evaluation_still_fails_when_nothing_matches(tmp_path, monkeypatch):
    ...
    from pe_ensemble.evaluation.jobs import create_job, get_job
    from pe_ensemble.evaluation.runner import EvaluationError
