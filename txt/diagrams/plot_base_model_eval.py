...
Defaults write into this directory (``txt/diagrams/``). With no CSV argument the
script plots both the unfiltered vendor heatmap and the design-rule stack, using
run pointers under ``scripts/experiments/base-model-eval/results/``.

Usage:
  python txt/diagrams/plot_base_model_eval.py
  python txt/diagrams/plot_base_model_eval.py --design
  python txt/diagrams/plot_base_model_eval.py path/to/paper_comparison.csv
  python txt/diagrams/plot_base_model_eval.py \\
    path/to/a.csv --stack path/to/b.csv \\
    --panel-titles "Hsu / OptiPrime design rules" "Anzalone 2019 design rules"

Font sizes: edit ``FS_*`` constants near the top of this file.
"""
...
DEFAULT_DIAGRAM_DIR = Path(__file__).resolve().parent
EVAL_RESULTS_DIR = REPO_ROOT / "scripts" / "experiments" / "base-model-eval" / "results"

# Fallbacks when pointer files are missing or stale.
_DEFAULT_HEATMAP_RUN = "20260912T203559Z"
_DEFAULT_DESIGN_OPTIPRIME_RUN = "20260912T120355Z-design-optiprime"
_DEFAULT_DESIGN_ANZALONE_RUN = "20260912T120355Z-design-anzalone"
_DEFAULT_DESIGN_TITLES = (
    "Hsu / OptiPrime design rules",
    "Anzalone 2019 design rules",
)
...
def _read_run_pointer(pointer_name: str, fallback_run: str) -> str:
    path = EVAL_RESULTS_DIR / pointer_name
    if path.is_file():
...
def resolve_eval_csv(pointer_name: str, fallback_run: str) -> Path:
    """Resolve ``…/results/<RUN_ID>/paper_comparison.csv`` via a pointer file."""
    run_id = _read_run_pointer(pointer_name, fallback_run)
    csv_path = EVAL_RESULTS_DIR / run_id / "paper_comparison.csv"
    if csv_path.is_file():
        return csv_path
    fallback = EVAL_RESULTS_DIR / fallback_run / "paper_comparison.csv"
    if fallback.is_file() and fallback != csv_path:
        return fallback
    raise SystemExit(
        f"Error: no paper_comparison.csv for {pointer_name} "
        f"(tried run {run_id!r} and fallback {fallback_run!r} under {EVAL_RESULTS_DIR})"
    )
...
BENCH_META = [
    ...
    ("anzalone-endo__pe2", "PE2", "Anzalone"),
    ("anzalone-endo__pe3", "PE3", "Anzalone"),
...
class FontSizes:
    """Typographic sizes (pt) for heatmap text elements."""

    cell: float = FS_CELL
    dataset: float = FS_DATASET
    xtick: float = FS_XTICK
    ytick: float = FS_YTICK
    legend: float = FS_LEGEND
    cbar_label: float = FS_CBAR_LABEL
    cbar_tick: float = FS_CBAR_TICK
    title: float = FS_TITLE


DEFAULT_FONTS = FontSizes()


class _NotScoredHandle:
    """Legend proxy for an unscored heatmap cell (grey fill + full-cell X)."""

    def get_label(self) -> str:
        return "Not scored"


class _NotScoredHandler(HandlerBase):
    def create_artists(
        self,
        legend: Any,
        orig_handle: Any,
        ...
        pad_y = 0.0
        line_kw = {
            "color": MISSING_X_COLOR,
            "linewidth": 1.2,
            "solid_capstyle": "butt",
            "transform": trans,
        }
        line1 = Line2D(
            [x0 + pad_x, x0 + width - pad_x],
            [y0 + pad_y, y0 + height - pad_y],
            **line_kw,
        )
        line2 = Line2D(
            [x0 + pad_x, x0 + width - pad_x],
            [y0 + height - pad_y, y0 + pad_y],
            **line_kw,
        )
        return [rect, line1, line2]


def _add_unscored_cell_cross(ax: Any, col: int, row: int) -> None:
    """Draw one X through all four corners of an unscored heatmap cell."""
    x0, x1 = col - 0.5, col + 0.5
    y0, y1 = row - 0.5, row + 0.5
    clip_box = TransformedBbox(Bbox.from_extents(x0, y0, x1, y1), ax.transData)
    line_kw = {
        "color": MISSING_X_COLOR,
        "linewidth": MISSING_X_LW,
        "solid_capstyle": "projecting",
        "zorder": 4,
        "clip_on": True,
    }
    for xs, ys in (([x0, x1], [y0, y1]), ([x0, x1], [y1, y0])):
        line = Line2D(xs, ys, **line_kw)
        ax.add_line(line)
...
def _draw_heatmap_panel(
    ...
    *,
    ...
    ax.tick_params(axis="x", length=0, labelsize=fonts.xtick, colors="#555555")
    ax.tick_params(axis="y", length=0, labelsize=fonts.ytick, colors="#555555")
    ...
    for i in range(n_models):
        for j in range(n_benches):
            ...
            if not np.isfinite(value):
                _add_unscored_cell_cross(ax, j, i)
                continue
...
def _add_heatmap_colorbar(
    ...
    *,
    ...
    cbar.set_label(label, fontsize=fonts.cbar_label, color="#444444")
    cbar.set_ticks(np.arange(vmin, vmax + 1e-9, 0.2))
    cbar.ax.tick_params(labelsize=fonts.cbar_tick, colors="#555555")
...
def _add_heatmap_legend(
    ...
    legend_handles: list[Any] = [_NotScoredHandle()]
    handler_map: dict[Any, Any] = {_NotScoredHandle: _NotScoredHandler()}
    if layout.name == "base":
...
def plot_heatmap(
    ...
    _add_heatmap_colorbar(
        ...
        vmin=vmin,
        vmax=vmax,
        label=layout.cbar_label,
        fonts=fonts,
    )
    _add_heatmap_legend(fig, layout, legend_anchor, fonts=fonts)
...
def plot_stacked_heatmaps(
    ...
    _add_heatmap_legend(fig, layouts[0], (0.46, 0.045), fonts=fonts)
    return _save_heatmap_figure(fig, out_path)


def _apply_metric_layout(
    *,
    layout_choice: str,
    metric_choice: str,
    inferred: PlotLayout,
) -> PlotLayout:
    if layout_choice == "base":
        layout = BASE_LAYOUT
    elif layout_choice == "scratch":
        layout = SCRATCH_LAYOUT
    else:
        layout = inferred
    if metric_choice == "pearson":
        return replace(layout, value_column="pearson_plot", cbar_label="Pearson r")
    if metric_choice == "spearman":
        return replace(layout, value_column="spearman_plot", cbar_label="Spearman R")
    return layout
...
def _render_heatmap(
    *,
    ...
    layout: PlotLayout,
    min_n: int,
    stack_paths: Optional[list[Path]] = None,
    ...
    if input_path.is_dir():
        ...
        rows = blank_small_n(scratch_to_comparison(cell_rows), min_n)
    elif input_path.is_file():
        rows = blank_small_n(load_comparison(input_path), min_n)
    else:
        raise SystemExit(f"Error: {input_path} not found")

    stack_paths = list(stack_paths or [])
    if stack_paths:
        titles = list(panel_titles or [])
        ...
        heat = plot_heatmap(rows, out_dir / heatmap_name, layout=layout, fonts=fonts)
    print(f"Wrote {heat}")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default (no args): plot HEATMAP_RUN_ID and the design-rule stack."
        ),
    )
    parser.add_argument(
        "--design",
        action="store_true",
        help=(
            "Plot only the stacked design-rule heatmaps "
            "(DESIGN_OPTIPRIME_RUN_ID + DESIGN_ANZALONE_RUN_ID)."
        ),
    )
    parser.add_argument(
        "--out-dir",
    ...
    parser.add_argument(
        ...
        "eval_pearson_heatmap_design_rules.pdf when --stack/--design is set).",
    )
    parser.add_argument(
        "--min-n",
        type=int,
    ...
    panel_titles = list(args.panel_titles) if args.panel_titles else None
    fonts = DEFAULT_FONTS
    apply_style()

    def layout_for(path: Path) -> tuple[PlotLayout, Path]:
        if path.is_dir():
            inferred, out_default = SCRATCH_LAYOUT, path / "figures"
        else:
            inferred, out_default = infer_layout(load_comparison(path)), DEFAULT_DIAGRAM_DIR
        layout = _apply_metric_layout(
            layout_choice=args.layout,
            metric_choice=args.metric,
            inferred=inferred,
        )
        return layout, args.out_dir or out_default

    # Default: both paper figures.
    if args.comparison_csv is None and not args.design and not stack_paths:
        unfiltered = resolve_eval_csv("HEATMAP_RUN_ID", _DEFAULT_HEATMAP_RUN).resolve()
        design_a = resolve_eval_csv(
            "DESIGN_OPTIPRIME_RUN_ID", _DEFAULT_DESIGN_OPTIPRIME_RUN
        ).resolve()
        design_b = resolve_eval_csv(
            "DESIGN_ANZALONE_RUN_ID", _DEFAULT_DESIGN_ANZALONE_RUN
        ).resolve()
        print(f"Unfiltered: {unfiltered}")
        layout, out_dir = layout_for(unfiltered)
        _render_heatmap(
            input_path=unfiltered,
            out_dir=out_dir,
            layout=layout,
            min_n=args.min_n,
        ...
        print(f"Design stack: {design_a}")
        print(f"             {design_b}")
        layout, out_dir = layout_for(design_a)
        _render_heatmap(
            input_path=design_a,
            out_dir=out_dir,
            layout=layout,
            min_n=args.min_n,
            stack_paths=[design_b],
            panel_titles=list(_DEFAULT_DESIGN_TITLES),
            fonts=fonts,
        )
        return 0

    if args.design:
        if args.comparison_csv is not None or stack_paths:
            raise SystemExit("Error: --design cannot be combined with a CSV path or --stack")
        input_path = resolve_eval_csv(
            "DESIGN_OPTIPRIME_RUN_ID", _DEFAULT_DESIGN_OPTIPRIME_RUN
        ).resolve()
        stack_paths = [
            resolve_eval_csv(
        ...
        panel_titles = list(_DEFAULT_DESIGN_TITLES)
        print(f"Design stack: {input_path}")
        print(f"             {stack_paths[0]}")
    else:
        input_path = args.comparison_csv.resolve()

    layout, out_dir = layout_for(input_path)
    if input_path.is_dir():
        cell_rows = load_scratch_cell_rows(input_path)
        if not cell_rows:
    ...
    _render_heatmap(
        ...
        layout=layout,
        min_n=args.min_n,
        stack_paths=stack_paths or None,
