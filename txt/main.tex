...
The matrix matches the vendor heatmap's per-condition columns: PRIDICT Library~1 (HEK293T PE2), Library-Diverse as three separate sheets (HEK, K562, K562-MLH1dn), DeepPrime ClinVar (HEK293T PE2), DeepPE HEK pooled HT/type/position/endo, MinsePIE HEK PE2 insertion screens, and OptiPrime Lib-MMR / Lib-CV as eight sheets (HEK293T/HeLa $\times$ PE2/PE4). That is $15$ datasheets $\times$ $3$ models $\times$ $3$ seeds $= 135$ train--eval repeats. Pooling Library-Diverse across cell lines or Lib-MMR / Lib-CV across PE system and cell line would confound MMR status with absolute efficiency scale---the same reason the vendor evaluation expands those panels. Jobs ran on ARC HTC L40S GPUs (typically short / 12\,h per seed; ClinVar OPED required resume of the Optuna study after timeouts).

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{diagrams/scratch_pearson_heatmap.pdf}
    \caption{From-scratch Pearson $r$ (mean $\pm$ s.d.\ over three holdout$_3$ seeds) for DeepPrime, OPED, and PRIDICT2. Higher is better. Library-Diverse is split by HEK / K562 / K562-MLH1dn, and Lib-MMR / Lib-CV by HEK293T/HeLa $\times$ PE2/PE4, matching \autoref{fig:pearson-heatmap}. Unlike that vendor matrix, every cell is an in-sheet train--test under the same protocol; no vendor weights or author-fold fills are used.}
...
\begin{figure}[ht]
    ...
    \caption{Same matrix as \autoref{fig:scratch-pearson-heatmap}, scored with Spearman $\rho$. Rank correlations remain high on Library-Diverse K562 even where Pearson falls, consistent with a compressed efficiency scale on that MMR-proficient line.}
...
MinsePIE HEK PE2 is the unstable column. Mean Pearson looks moderate for DeepPrime ($0.56$) and weaker for OPED and PRIDICT2 ($0.35$ / $0.24$), yet seed-to-seed scatter is large (DeepPrime Pearson s.d.\ $0.44$; PRIDICT2 $0.26$). The pooled insertion screens concentrate on a handful of endogenous loci, so a 15\% group holdout can change composition dramatically across seeds; the means alone are not reliable rankings.

On Library-Diverse, HEK and K562-MLH1dn remain high for DeepPrime ($r = 0.83$ / $0.81$) and PRIDICT2 ($r = 0.87$ / $0.81$), while K562 is lower in Pearson ($r = 0.67$ / $0.60$) with Spearman still high ($\rho = 0.78$ / $0.79$). That follows the compressed K562 efficiency scale (median intended-edit ${\sim}0.8\%$ versus ${\sim}11\%$ on HEK): the models rank pegRNAs after training on K562, but Pearson is harder on a low-efficiency assay. OPED follows the same HEK $\approx$ MLH1dn $>$ K562 order at lower absolute $r$ ($0.64$ / $0.62$ / $0.44$).

Performance on Lib-MMR is relatively flat across HEK/HeLa $\times$ PE2/PE4 ($r \approx 0.50$--$0.55$ for DeepPrime and PRIDICT2; $r \approx 0.39$--$0.41$ for OPED). Lib-CV is higher and shows a PE4 lift for all three models (DeepPrime HEK $0.46 \to 0.61$, HeLa $0.50 \to 0.62$; PRIDICT2 $0.54 \to 0.60$ / $0.50 \to 0.62$). Absolute Pearson on both Hsu panels remains well below the large HEK reporter sheets.
...
% The published-checkpoint heatmap and the from-scratch training matrix address complementary questions. The former evaluates released models under author-defined protocols with safeguards against data leakage; the latter compares architectures when PE Hub controls the data split, hyperparameter search, and checkpoint registration. On the large HEK reporter datasets evaluated as individual conditions, DeepPrime and PRIDICT2 performed more strongly under a shared Optuna search budget. Results on MinsePIE were more sensitive to the random seed, consistent with the small number of endogenous loci and the resulting variation introduced by a 15\% group holdout. OptiPrime is not included in these comparisons because PE Ensemble does not currently provide a from-scratch search space for the model.
To isolate the effect of architecture on prime-editing prediction, the models were retrained from random initialization with PE Hub controlling the data split, hyperparameter search, and checkpoint registration. On the large HEK reporter sheets scored as individual conditions (Library~1, ClinVar, DeepPE, and Library-Diverse HEK), DeepPrime and PRIDICT2 performed more strongly under a shared Optuna budget, and OPED lagged throughout. 
Because each Library-Diverse cell is trained in-sheet, the K562 drop is not a vendor-style MMR transfer mismatch. HEK and K562-MLH1dn remain easy to fit because those MMR-deficient assays have high editing efficiencies; K562 compresses the label scale (median intended-edit ${\sim}0.8\%$ versus ${\sim}11\%$ on HEK). Pearson fell on K562 while Spearman stayed high, so the models still ranked pegRNAs after training on that line but were less well calibrated to its low absolute efficiencies. Training in-sheet on Hsu's eight HEK/HeLa $\times$ PE2/PE4 cells did not raise those libraries to HEK-reporter levels. Lib-MMR was essentially flat across conditions, including HeLa PE2, where vendor DeepPrime had failed on \texttt{NNNN}-padded sequence; Lib-CV showed a PE4 lift for all three models. Results on MinsePIE were more sensitive to the random seed, consistent with the small number of endogenous loci and the variation introduced by a 15\% group holdout. OptiPrime is omitted from this comparison because PE Ensemble does not yet provide a from-scratch search space for it.

The scope of these conclusions is constrained by the available data. Approximately 92\% of catalog entries are synthetic reporter designs, and 97\% were assayed in vitro. Anzalone and MinsePIE provide the principal endogenous evaluations, but both covering relatively few loci. In addition, Hsu’s protospacer cross-validation folds are absent from the deposited tables, preventing an equivalent in-domain evaluation. These cells are therefore left blank in the heatmap, and diagnostic results with potential train–test overlap should not be used to rank models. Differences in outcome definitions impose further limits on comparison: Koeppel, for example, reports successful insertions relative to total insertions, without a recoverable background rate. The platform itself also has a defined scope. The interactive designer currently supports a single \texttt{(pre/after)} edit and SpCas9 NGG nick sites on the supplied target strand, while plugin registration has been implemented but has yet to be demonstrated with a third-party architecture.

Although therapeutic prime editing motivates this work, the available evidence remains dominated by HEK and K562 reporter assays on man-made target sequences. The transfer results nevertheless offer practical guidance: model selection should account for MMR status, PE2 performance should not be assumed to extend to PE3, and long insertions should be treated seperately. For developers of new predictors, PE Hub provides a common route into this evaluation framework through a model wrapper and a PE-core converter, reducing the need to reconstruct datasets and maintain separate evaluation scripts. Priorities for further development include supporting OptiPrime training if its base weights become available, and demonstrating external model integration through a public plugin case study. More broadly, chromatin-aware and primary-cell screens are needed to establish how well current predictors generalize to the therapeutic settings that motivate their use, and updates to PE-Hub is needed to support these more sophisticated studies.
