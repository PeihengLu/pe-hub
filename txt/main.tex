...
Although the ability to join sheets from different studies is implemented, it is not always sensible to do so. The editing efficiency measurement produced by different studies under different experimental conditions are often not directly comparable on the same scale\cite{schwankgeraldPredictingPrimeEditing2023}.
...
OptiPrime was not helped by either filter, with a correlation drop on both DeepPE and Anazalone PE2. Their performance on MinsePIE inserts did improve in terms of linear correlation, but spearman rank suffered a slight drop($r = 0.10 \to 0.29$, $\rho = 0.52 \to 0.49$, $n = 13{,}826$).
