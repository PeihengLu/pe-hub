...
Under that PBS/RTT filter, Hsu's Lib-MMR / Lib-CV panels are essentially intact (${\sim}$95--100\% of DeepPrime $n$). The cut falls on external sheets instead: DeepPE HEK leftover-excluded $n$ for DeepPrime drops ($1{,}033 \to 155$), ClinVar's leftover-excluded author test shrinks further ($28{,}221 \to 1{,}343$), and Library~1 / Library-Diverse keep about one-sixth of their unfiltered holdouts. Anzalone PE2 keeps $n = 68$ of $199$ (DeepPrime $0.72 \to 0.69$; PRIDICT2 HEK $0.63 \to 0.63$; OptiPrime $0.64 \to 0.59$); PE3 ($n = 100$) collapses toward zero for every model (OptiPrime $r = -0.10$). Pearson values on the large libraries still move little in a consistent direction. DeepPrime is slightly down on DeepPE ($0.61 \to 0.57$), almost unchanged on ClinVar and Library~1, and slightly down on Library-Diverse HEK ($0.56 \to 0.47$). OPED falls modestly on DeepPE ($0.76 \to 0.72$) and collapses on the remaining MinsePIE inserts ($0.41 \to 0.07$), while rising on Library~1 ($0.28 \to 0.46$). OptiPrime tracks a small DeepPE drop ($0.44 \to 0.39$, $n = 1{,}033$) but is the only model that still ranks the filtered MinsePIE inserts ($r = 0.10 \to 0.29$, $\rho = 0.52 \to 0.49$, $n = 13{,}826$), while DeepPrime and OPED do not. PRIDICT2's in-domain Diverse HEK cell softens slightly ($0.90 \to 0.85$), and transfer onto Hsu's libraries shifts by only a few hundredths.

Anzalone's 10--16\,nt RTT window is a different cut (\autoref{fig:pearson-heatmap-design-rules}b). It empties MinsePIE (median insert 18\,nt), and keeps only 3--8\% of Lib-MMR / Lib-CV, leaving $n \approx 50$--$160$ on those Hsu sheets---too small to read ranking changes. The Anzalone PE2 column itself shrinks to $n = 26$ (too small to interpret); PE3 keeps $n = 43$--$44$ and DeepPrime rises ($0.30 \to 0.69$), which should not be over-read. On the larger external remainders the picture is again mixed: DeepPE OPED falls ($0.76 \to 0.63$), ClinVar PRIDICT2 HEK rises ($0.54 \to 0.62$), and DeepPrime on Library-Diverse HEK improves ($0.56 \to 0.64$). OptiPrime was not re-scored under this RTT-window cut after the target-geometry restore.
...
\begin{figure}[p]
    ...
    \caption{(a)~Pearson $r$ (mean $\pm$ s.d.\ over three holdout$_3$ seeds).
    (b)~Spearman $\rho$ on the same matrix; rank correlations are often higher than Pearson on the pooled Library-Diverse and Hsu columns, where efficiency scales differ across edit classes and (in this pilot) across cell lines or PE systems.
    Library-Diverse and Lib-MMR / Lib-CV columns still pool conditions (see text); Library~1, ClinVar, DeepPE, and MinsePIE are single-condition. Unlike \autoref{fig:pearson-heatmap}, every cell is an in-sheet train--test under the same protocol; no vendor weights or author-fold fills are used.}
