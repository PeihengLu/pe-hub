...
Large-scale prime editing studies have produced a growing body of data, but differences in input formats and experimental annotations leaves their usage with substaintial overhead. PE-DB was developed to make these datasets accessible through a common representation, and brings together guide RNA sequences, target DNA sequences, and measured editing outcomes from these experiments.
...
\autoref{fig:data_composition} summarizes the composition of data from the studies. In total, there are ${\sim}$756,000 pegRNA--target pairs across seven studies, including 477 Easy-Prime-reformatted Anzalone endogenous designs.
...
Model-format conversion is implemented centrally in PE-DB. DeepPrime receives a 74-base wild-type window with the protospacer starting at offset four, a masked 74-base edited representation containing the PBS and RTT, and its engineered sequence features. These include lengths, edit indicators, GC composition, melting temperatures, RNA minimum free energies, and the DeepSpCas9 score. OPED receives the full unpadded wild-type target already present on the standardized row (DeepPE reporters remain 47~bp; endogenous Anzalone rows expanded to Liu's 350~bp hg38 window), a PBS extracted from the wild-type sequence, and an RT sequence extracted from the mutant sequence. Insertion-alignment padding is removed before constructing the unedited target, preventing an inserted allele from entering the wild-type input. The converter does not invent missing flanks (no fixed crop to 47~bp and no \texttt{A}/\texttt{N} padding to a target length).

The PRIDICT and PRIDICT2 format names invoke the same PRIDICT2 conversion pipeline. It constructs initial and mutated target sequences, edit positions and lengths, region annotations, and thermodynamic features, including seven ViennaRNA minimum-free-energy features for the spacer, scaffold, PBS, RT, and their combinations. Extended endogenous windows are shifted into the author's coordinate frame, with the spacer at offset ten. OptiPrime receives the spacer and reverse-complemented PBS and RTT in the RNA alphabet, together with unedited and edited DNA contexts aligned to Hsu's four-base upstream protospacer frame (short reporter windows without upstream sequence are left-padded with \texttt{A}). Assay metadata---scaffold, epegRNA motif, Cas9 variant, PE type, integration time, and kinetic \texttt{group}---are set from the source study and cell line so that trained group factors load correctly, rather than using a single Hsu-default for every sheet.

Where the source lacks sufficient flanking sequence, conversion follows model-specific conventions rather than inventing DNA for OPED: DeepPrime uses \texttt{N} for missing target context and \texttt{X} for masked edited positions when a 74~bp window cannot be filled from the source. OPED simply passes through whatever unpadded WT length is available. Converted frames are cached on disk with a converter revision check to avoid repeating expensive feature calculations. Missing or non-numeric efficiency labels are removed during standardization, while measured zeros are retained; model training rejects missing labels that reach a wrapper through an alternative input path.
...
Using one of the two pegRNA design rulesets, users can leverage models trained or shipped with PE Ensemble, even custom ensembles from existing models, to design and rank pegRNAs in their provided locis for their desirable edits.  
...
\begin{figure}[ht]
    ...
    \caption{Pearson $r$ between vendor-model predictions and measured intended-edit efficiency. Columns are study panels; rows are weight sets. The first row includes Anzalone 2019 HEK293T PE2 and PE3 (full Easy-Prime-reformatted sheet). Cells marked with an X and hatched cells cannot be measured in PE-Hub due to siginificant in-domain leakage. If author reported value can be found in their respected paper, it is filled into the heatmap in hatched cell indicating author-reported fill. If no meaningful result can be found, the model is marked with X and is not scored.}
...
DeepPrime's Spearman on these sheets ($R = 0.79$ / $0.59$ / $0.75$) is in the same range as Yu's $\leq 3$~bp subset of Library-Diverse; Pearson is lower because the unfiltered library includes edits outside DeepPrime's 1--3~bp training support. OptiPrime is again blank on the heatmap (Hsu trained on this library); the leaky diagnostic stays at $r \approx 0.13$--$0.20$. OPED remains a modest transfer model ($r \approx 0.26$--$0.31$).

\paragraph{MinSePIE HEK PE2.}
This column is almost entirely insertions (median 18~nt, RT templates often $>50$~nt) at a handful of endogenous loci, still in HEK293T PE2. DeepPrime reaches $r = 0.68$ on the leftover-excluded subset ($n = 13{,}826$). OPED ($r = 0.41$) and OptiPrime ($r = 0.48$) retain a usable ranking on the smaller PE-Hub holdout ($n = 4{,}301$), consistent with OPED's nucleotide decoder not being hard-capped at 3~bp and with OptiPrime's kinetic features still responding to PBS/RT length once assay metadata and group factors are study-aware. The PRIDICT2 surprise is that the HEK head ($r = 0.45$) only modestly beats the K562 head ($r = 0.34$) on a HEK MMR-deficient sheet. If head--MMR matching were the dominant effect, HEK should win by more. The more plausible reading is edit class: PRIDICT2's library-diverse insertions stop at 15~nt (median 7), whereas MinSePIE is a long-insert, few-locus endogenous assay. The heatmap cell is not an RC or HAP1 mix-up; those panels were excluded.

\paragraph{Anzalone 2019 HEK PE2 and PE3.}
This is the small arrayed endogenous set, scored in full rather than on a 20\% holdout. Wide targets for DeepPrime are Easy-Prime amplicon crops to the 74\,bp layout (no 3$'$ \texttt{N} pad); OPED receives the full 350\,bp hg38 sense window used by Liu et al.\ Fig.~2i. PE2 keeps every Easy-Prime row ($n = 199$). PE3 leftover-excludes loci that overlap vendor training spacers ($n = 213$ of $278$ for DeepPrime, OptiPrime, and PRIDICT2; OPED keeps all $278$). DeepPrime reaches $r = 0.72$ / $\rho = 0.69$ on PE2 against Yu et al.\ $r = R = 0.74$ (Fig.~3H)\cite{yuPredictionEfficienciesDiverse2023}. PRIDICT2 HEK ($r = 0.63$, $\rho = 0.69$) matches Mathis et al.\ 2024 on Anzalone ($r = 0.63$, $R = 0.69$, $n = 181$). The K562 head is similar in Pearson ($r = 0.66$). With the corrected OptiPrime assay wiring, OptiPrime reaches $r = 0.23$ on PE2; OPED rises to $r = 0.46$ once the 350\,bp genomic window is passed through without inventing flanks. PE3 remains a different editor: every model falls to $r \approx 0.00$--$0.30$.
...
\paragraph{OptiPrime leaky in-domain check.}
Because Hsu's protospacer CV folds are not in the deposited tables, the heatmap cannot show OptiPrime on its own training sheets. To check that the PE-DB converter and JAX wrapper are not simply broken, we re-scored those sheets with \nolinkurl{allow\_data\_leak} (Methods): a PE-Hub locus-grouped holdout on Lib-MMR / Lib-CV / Library~1 / Library-Diverse, and Yu's ClinVar author test fold kept in full. These numbers are optimistic by construction and are not heatmap cells.

On Hsu's own libraries the wiring looks right. HeLa PE4 reaches $r = 0.76$ / $0.75$ on Lib-MMR / Lib-CV ($n \approx 1{,}900$--$2{,}100$), close to the hatched Fig.~4 fills ($0.760$ / $0.803$). HeLa PE2 is lower ($r = 0.57$ on both libraries versus the pooled fill $0.760$); HEK PE2/PE4 sit at $r \approx 0.55$--$0.63$. The eight-cell mean is $r \approx 0.63$ against Hsu's four-condition mean $0.723$\cite{hsuMechanisticMachineLearning2026}. Spearman remains strong throughout ($\rho \approx 0.67$--$0.77$).

The external training corpora stay weak even with leakage: ClinVar $r = 0.42$ ($n = 28{,}221$), Library~1 $r = 0.16$ ($n = 18{,}399$), and Library-Diverse $r \approx 0.13$--$0.20$ across HEK / K562 / MLH1dn. So the remaining OptiPrime gap is concentrated on Kim/Schwank-format sheets (assay metadata and group factors), not on a total failure of the Hsu Lib-MMR / Lib-CV path that the paper figures use.

\subsubsection{Design-rule filtered evaluation}

Hsu et al.\ argued that many published pegRNAs would never be ordered in practice, and that correlations for other models may therefore look better than they should~\cite{hsuMechanisticMachineLearning2026}. We repeated the unfiltered matrix of \autoref{fig:pearson-heatmap} under the two named PE-DB rulesets (Methods): Hsu's PBS and RTT-start constraints (\nolinkurl{optiprime}) and the Anzalone et al.\ 2019 starting recommendations (\nolinkurl{anzalone})~\cite{anzaloneSearchandreplaceGenomeEditing2019}. Layout and colour scale are unchanged; only which rows are scored changes (\autoref{fig:pearson-heatmap-design-rules}). OptiPrime is scored on the same filtered sheets as the deep models (in-domain blanks and author fills unchanged).
...
Under that PBS/RTT filter, Hsu's Lib-MMR / Lib-CV panels are essentially intact (${\sim}$95--100\% of DeepPrime $n$). The cut falls on external sheets instead: DeepPE HEK leftover-excluded $n$ for DeepPrime drops ($1{,}033 \to 155$), ClinVar's author-test remainder shrinks further ($1{,}367 \to 87$), and Library~1 / Library-Diverse keep about one-sixth of their unfiltered holdouts. Anzalone PE2 keeps $n = 68$ of $199$ (DeepPrime $0.72 \to 0.69$; PRIDICT2 HEK $0.63 \to 0.63$; OptiPrime $0.23 \to 0.31$); PE3 ($n = 100$) collapses toward zero for every model. Pearson values on the large libraries still move little in a consistent direction. DeepPrime is slightly down on DeepPE ($0.61 \to 0.57$), almost unchanged on ClinVar and Library~1, and slightly down on Library-Diverse HEK ($0.56 \to 0.47$). OPED falls modestly on DeepPE ($0.76 \to 0.72$) and collapses on the remaining MinSePIE inserts ($0.41 \to 0.07$), while rising on Library~1 ($0.28 \to 0.46$). OptiPrime tracks the same DeepPE drop ($0.43 \to 0.37$) but holds on MinSePIE ($0.48 \to 0.50$), the one filtered long-insert cell where the mechanistic model stays usable while DeepPrime and OPED do not. PRIDICT2's in-domain Diverse HEK cell softens slightly ($0.90 \to 0.85$), and transfer onto Hsu's libraries shifts by only a few hundredths.

Anzalone's 10--16\,nt RTT window is a different cut (\autoref{fig:pearson-heatmap-design-rules}b). It empties MinSePIE (median insert 18\,nt), and keeps only 3--8\% of Lib-MMR / Lib-CV, leaving $n \approx 50$--$160$ on those Hsu sheets---too small to read ranking changes. The Anzalone PE2 column itself shrinks to $n = 26$ (too small; OptiPrime $r = -0.06$); PE3 keeps $n = 43$--$44$ and both DeepPrime and OptiPrime rise ($0.30 \to 0.69$ and $0.01 \to 0.51$), which should not be over-read. On the larger external remainders the picture is again mixed: DeepPE OPED falls ($0.76 \to 0.63$) and OptiPrime falls ($0.43 \to 0.35$), ClinVar PRIDICT2 HEK rises ($0.54 \to 0.62$), and DeepPrime on Library-Diverse HEK improves ($0.56 \to 0.64$).

So Hsu is right that many published designs fail simple practitioner checks on PBS and RTT start base, and the $n$ drop on external libraries shows that. What the filtered matrices do \emph{not} support is the stronger claim that other models look good mainly because of those designs: removing them helps some cells and hurts others, including OptiPrime. We also do not treat departures from $9+L$ / $19+L$ on Hsu's own sheets as a reason to call those pegRNAs poorly designed.

\subsection{From-scratch base-weight training}

The vendor matrix above scores published checkpoints. To ask how the three deep architectures compare when each is trained from random initialization on the same sheet and the same holdout protocol, we ran a second matrix on Oxford ARC: DeepPrime, OPED, and PRIDICT2, each trained without loading vendor weights (\nolinkurl{load\_pretrained=false}). OptiPrime is omitted because PE Ensemble does not yet expose a from-scratch Optuna search space for it; its Lib-MMR and Lib-CV sheets are still included as training sets for the other three models.

\paragraph{Protocol.}
Every cell uses the datasheet-benchmark runner under a fixed \nolinkurl{holdout\_3} split (70/15/15 train/validation/test by locus group, base seed 42). For each of three split seeds (42, 43, 44) the job runs ten Optuna trials, registers weights from the best trial, and evaluates on that seed's held-out test partition in the same SLURM allocation. Epoch budgets are not searched: early stopping (patience 10) is the primary stop, with a large \nolinkurl{max\_epochs} backstop. DeepPrime's Optuna objective is negative validation loss; OPED and PRIDICT2 maximize validation Spearman. Unlike the vendor evaluation, author folds are not used: Library~1, which has no deposited PRIDICT holdout, becomes a fair in-sheet comparison for all three models, and ClinVar / DeepPE are scored on the same random holdout rather than on each paper's proprietary test definition.

The intended matrix matches the vendor heatmap's per-condition columns: PRIDICT Library~1 (HEK293T PE2), Library-Diverse as three separate sheets (HEK, K562, K562-MLH1dn), DeepPrime ClinVar (HEK293T PE2), DeepPE HEK pooled HT/type/position/endo, MinSePIE HEK PE2 insertion screens, and OptiPrime Lib-MMR / Lib-CV as eight sheets (HEK293T/HeLa $\times$ PE2/PE4). That is $15$ datasheets $\times$ $3$ models $\times$ $3$ seeds $= 135$ train--eval repeats. Pooling Library-Diverse across cell lines or Lib-MMR / Lib-CV across PE system and cell line would confound MMR status with absolute efficiency scale---the same reason the vendor evaluation expands those panels.

The heatmaps below are from an earlier ARC run that still pooled Library-Diverse (${\sim}$66k) and each Hsu library (${\sim}$36k / ${\sim}$37k) into single cells; Library~1, ClinVar, DeepPE, and MinSePIE used the intended single-condition filters. Jobs ran on ARC HTC L40S GPUs (typically short / 12\,h per seed; ClinVar OPED required resume of the Optuna study after timeouts). A corrected per-condition re-run for Library-Diverse and the eight Hsu datasheets is the next step before reading MMR- or cell-line-specific scratch rankings.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{diagrams/scratch_pearson_heatmap.pdf}
    \caption{From-scratch Pearson $r$ (mean $\pm$ s.d.\ over three holdout$_3$ seeds) for DeepPrime, OPED, and PRIDICT2. Higher is better. Library-Diverse and Lib-MMR / Lib-CV columns in this figure still pool conditions (see text); Library~1, ClinVar, DeepPE, and MinSePIE are single-condition. Unlike \autoref{fig:pearson-heatmap}, every cell is an in-sheet train--test under the same protocol; no vendor weights or author-fold fills are used.}
    \label{fig:scratch-pearson-heatmap}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{diagrams/scratch_spearman_heatmap.pdf}
    \caption{Same matrix as \autoref{fig:scratch-pearson-heatmap}, scored with Spearman $\rho$. Rank correlations are often higher than Pearson on the pooled Library-Diverse and Hsu columns, where efficiency scales differ across edit classes and (in this pilot) across cell lines or PE systems.}
    \label{fig:scratch-spearman-heatmap}
\end{figure}

\paragraph{Results.}
\autoref{fig:scratch-pearson-heatmap} and \autoref{fig:scratch-spearman-heatmap} summarize test Pearson and Spearman.

On PRIDICT Library~1, DeepPrime and PRIDICT2 are essentially tied (Pearson $r = 0.84 \pm 0.01$ and $0.83 \pm 0.00$; Spearman $\rho \approx 0.83$ / $0.80$), while OPED lags ($r = 0.63$, $\rho = 0.62$). That ordering is the opposite of Liu et al.'s claim that OPED reaches $r = 0.91$ after retraining on Library~1~\cite{liuDesignPrimeeditingGuide2023}: under a shared holdout and a common Optuna budget, OPED does not dominate this sheet.

DeepPrime ClinVar remains DeepPrime's strongest large-domain cell ($r = 0.78 \pm 0.01$, $\rho = 0.80$), close to the vendor ClinVar in-domain Pearson of $0.83$ in \autoref{fig:pearson-heatmap} despite the protocol change (author fold versus random holdout; leftover-exclusion off). PRIDICT2 is competitive ($r = 0.74$, $\rho = 0.80$); OPED trails ($r = 0.57$, $\rho = 0.59$). On DeepPE HEK, DeepPrime again leads ($r = 0.76$, $\rho = 0.80$), with PRIDICT2 next ($0.70$ / $0.76$) and OPED third ($0.66$ / $0.69$)---so the architecture that was trained on DeepPE by Liu et al.\ is not the best from-scratch fit under this budget.

MinSePIE HEK PE2 is the unstable column. Mean Pearson looks moderate for DeepPrime and OPED ($0.56$ / $0.67$), yet seed-to-seed standard deviations exceed $0.2$ (DeepPrime Pearson s.d.\ $0.44$), and PRIDICT2 collapses on average ($r = 0.24$). The pooled insertion screens concentrate on a handful of endogenous loci, so a 15\% group holdout can change composition dramatically across seeds; the means alone are not reliable rankings.

The pooled Library-Diverse and Hsu columns should be read cautiously. Library-Diverse shows a large Pearson--Spearman gap for DeepPrime and PRIDICT2 ($r \approx 0.64$ versus $\rho \approx 0.79$), and the Hsu libraries look modest overall ($r \approx 0.30$--$0.48$) with Lib-CV slightly kinder than Lib-MMR. Because those cells mix HEK/K562(/MLH1dn) or HEK/HeLa $\times$ PE2/PE4, they cannot separate MMR-matched behaviour the way \autoref{fig:pearson-heatmap} does; the corrected per-condition matrix is required before comparing architectures on those panels.

Overall, when vendor provenance and author folds are removed, DeepPrime and PRIDICT2 are the stronger general learners on the large HEK reporter sheets that were scored correctly (Library~1, ClinVar, DeepPE), OPED is consistently weaker under the same ten-trial budget, and MinSePIE remains noisy. The vendor heatmap and this scratch matrix answer different questions: the former measures published checkpoints under leak-safe author protocols; the latter measures how far each architecture can go when PE Hub owns the split, the search space, and the weight registry end to end.

\subsection{End-to-end PRIDICT2.0 reproduction}

To showcase PE-Hub's complete \emph{base training + transfer + ensemble} pipeline, we reproduced Mathis et al.'s PRIDICT2.0 construction on Oxford ARC using the reproduction scripts under \nolinkurl{scripts/experiments/pridict2-reproduction/}\cite{mathisMachineLearningPrediction2024}. The experiment was only performed once with the following protocol to avoid me biasing the result towards the original paper's numbers.
...
\paragraph{Protocol.}
Two bases were trained from scratch under a shared \nolinkurl{holdout\_3} recipe (70/15/15 by locus group): Model~A on PRIDICT Library~1 alone, and Model~B on Library~1 merged with DeepPrime ClinVar (ClinVar author fold $-1$ held out as test). Each base was then fine-tuned separately on Library-Diverse HEK and K562 for each author fold $x \in \{0,\ldots,4\}$, holding fold~$x$ out as test and splitting the remainder 70/15/15 for train/validation (\nolinkurl{num\_epochs}$=100$ with early-stopping patience~10; encoder frozen, decoder trainable; single edit-efficiency head with \nolinkurl{MSEloss}). That yields ten Model~A and ten Model~B checkpoints. For each cell line and fold we mean-ensembled the matching A/B pair and scored the held-out author fold ($n \approx 4{,}520$ per fold)---the same fold-matched evaluation used for vendor \nolinkurl{run\_x} weights earlier in this section. Library~1 has no deposited PRIDICT holdout, so the base stage uses PE Hub's random split rather than ``train on every locus'' as in the vendor provenance; fine-tune and ensemble stages follow the published Library-Diverse CV topology.

\paragraph{Results.}
\autoref{tab:pridict2-repro} summarises fivefold mean $\pm$ s.d.\ on Library-Diverse against Mathis et al.\ Fig.~1o--p. On HEK the reproduction matches the paper within a few hundredths (Pearson $r = 0.896 \pm 0.007$ versus $0.90$; Spearman $\rho = 0.916 \pm 0.002$ versus $0.91$). On K562 Spearman is close ($\rho = 0.795 \pm 0.004$ versus $0.81$) while Pearson is lower ($r = 0.620 \pm 0.008$ versus $0.70$). Absolute efficiencies on K562 are much smaller than on HEK (median intended-edit ${\sim}0.8\%$ versus ${\sim}11\%$ in the Diverse sheets), so Pearson is more sensitive to calibration and to the MSE-only head used here; rank agreement remains in the published range. Fold-to-fold scatter is small on both lines (Pearson s.d.\ $<0.01$), indicating a stable pipeline rather than a single lucky fold.
