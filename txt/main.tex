...
Although the ability to join sheets from different studies is implemented, it is not always sensible to do so.
...
Users can generate SpCas9 pegRNAs for a sequence they provide, rather than scoring a published library. The target-strand DNA is annotated with a single \texttt{(pre/after)} edit: a substitution such as \texttt{ATCG(A/G)TCG}, an insertion \texttt{ATCG(/GGG)TCG}, or a deletion \texttt{ATCG(AAA/)TCG}. The designer locates forward-strand NGG PAM sites whose nick (between spacer bases 17 and 18) lies at or 5$'$ of the edit and no farther than a configurable distance (40~bp by default). For each reachable nick it enumerates PBS and RTT geometries under one of the two named policies (\nolinkurl{optiprime} or \nolinkurl{anzalone}).

The \nolinkurl{optiprime} design policy uses PBS length 13 and Hsu's 3$'$ homology of $9+L$ (substitutions) or $19+L$ (insertions and deletions), with the C-avoiding length nudge; this is stricter than the named evaluation filter of the same name, which omits homology. The \nolinkurl{anzalone} policy enumerates PBS lengths 10--16~nt with 40--60\% GC content and RTT lengths 10--16~nt, again requiring that the first pegRNA RTT nucleotide is not C. Candidates that cannot cover the edit, that run off the supplied sequence, or that start the 3$'$ extension with C are discarded. The surviving rows are written in the PE-core schema.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{diagrams/scratch_pearson_heatmap.pdf}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{diagrams/scratch_spearman_heatmap.pdf}
...
The central empirical claim of this work is not that one published predictor is uniformly best, but that a shared schema, leak-safe splits, and registered checkpoints are enough to make those predictors commensurable. Where an author protocol can be recovered, PE Hub reproduces it. DeepPrime on the ClinVar test fold, the two PRIDICT2 heads on Library-Diverse HEK293T and K562, and OPED on Anzalone PE2 all land on the published Pearson values. Remaining blanks and drops are therefore protocol differences or domain shift, not a loading error.

The same toolchain can re-run a non-trivial training recipe rather than only score a shipped checkpoint. Reproducing PRIDICT2.0 from base pretraining through transfer fine-tuning and mean ensembling matched Mathis et al.\ on HEK293T ($r = 0.896$ versus $0.90$; $\rho = 0.916$ versus $0.91$). On K562, Spearman remained close ($\rho = 0.795$ versus $0.81$) while Pearson was lower ($r = 0.620$ versus $0.70$). That shortfall is the expected cost of protocol differences already built into the reproduction: Library~1 has no deposited holdout, so the base stage uses a random locus-grouped split rather than pretraining on every locus, and the fine-tune uses a single-efficiency MSE head rather than the authors' multi-outcome stack. Absolute efficiencies on K562 are an order of magnitude smaller than on HEK293T, so Pearson is more sensitive to calibration than rank. The experiment is not a new state of the art; it is evidence that PE-DB export, PE Ensemble train/tune/evaluate/ensemble, and fold-matched splits can carry a published pipeline end to end.

Transfer across those same sheets is a different question, and it is where a practitioner would actually use a model. Performance follows experimental context more closely than architecture. The PRIDICT2 HEK head collapses on MMR-proficient K562 and recovers when MLH1dn phenocopies deficiency, and the same PE2/PE4 direction appears on Hsu's HeLa screens. DeepPrime remains competitive on short HEK reporter edits and retains a rank signal on long MinsePIE insertions, but Pearson falls once the library leaves its 1--3~bp training support. Assay representation can erase a model without changing biology: DeepPrime's near-zero HeLa PE2 score on Lib-MMR is the \texttt{NNNN}-padded WT74, whereas Lib-CV, which supplies real 4~bp flanks, is higher in every matched cell. PE3 is a further editor, not a small variant of PE2; every model falls to $r \approx 0$--$0.30$ on Anzalone PE3. Published headline numbers are therefore in-domain scores. A decoder trained in HEK293T reporters is not entitled to that figure on a proficient cell line, a long insertion, or a nicking-guide protocol.

The vendor heatmap and the from-scratch matrix should be read as complementary rather than as a single ranking. The former measures published checkpoints under leak-safe author protocols; the latter asks how far each architecture can go when PE Hub owns the split, the search space, and the weight registry. On the large HEK reporter sheets that were scored as single conditions, DeepPrime and PRIDICT2 are the stronger general learners under a shared Optuna budget, while MinsePIE remains seed-unstable because a 15\% group holdout can reshuffle a handful of endogenous loci. Several scratch cells still pool Library-Diverse or Hsu conditions, so they cannot yet support MMR- or cell-line-specific architecture comparisons, and OptiPrime has no from-scratch search space in PE Ensemble.

These conclusions are bounded by what the catalog actually contains. About 92\% of entries are synthetic reporter designs and 97\% were assayed in vitro; Anzalone and MinsePIE are the main endogenous checks, and MinsePIE concentrates on a few loci. OptiPrime is integrated for inference only: the HetFormer pretraining weights were not released, and Hsu's protospacer CV folds are not in the deposited tables, so in-domain OptiPrime cells stay blank on the heatmap and leaky diagnostics must not be treated as a league table. Efficiency labels are not interchangeable (Koeppel reports successful insertions over total insertions, without a recoverable background rate). The interactive designer currently supports one \texttt{(pre/after)} edit and SpCas9 NGG nicks on the supplied target strand. Plugin registration is implemented but not demonstrated here with a third-party architecture.

The introduction framed pegRNA ranking as a bottleneck for therapeutic prime editing. The measurements that exist are still mostly HEK and K562 reporters. The practical implication of the transfer matrix is therefore modest and specific: match the model to mismatch-repair status, do not treat PE2 checkpoints as PE3 models, and treat long insertions as a separate problem. For groups publishing a new architecture, the intended use of PE Hub is to drop a wrapper and an optional PE-core converter onto this matrix rather than to rebuild ClinVar, Library~1, and a private evaluation script. A corrected per-condition scratch heatmap, OptiPrime training if base weights appear, and a public plugin case study are the obvious next measurements; chromatin-aware and primary-cell screens remain the larger gap between current predictors and the rare-disease use case that motivated the work.
